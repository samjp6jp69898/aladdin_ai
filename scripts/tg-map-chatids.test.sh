#!/usr/bin/env bash
# tg-map-chatids.test.sh — 全離線測試（不打 Telegram API、不碰真實 DB）
# 跑法：bash scripts/tg-map-chatids.test.sh
# 紀律：用 TG_GETUPDATES_CMD stub 掉 getUpdates、用 TG_REGISTRY_CLI stub 掉
#       telegram-dispatcher 的名冊 CLI。
#
# 2026-09-16（Phase 6：tech-users.csv 刪檔退役）：本檔改用假 registry CLI。
# 先前版本用 TG_NOTIFY_CSV 餵 fixture CSV、但 TG_REGISTRY_CLI 仍是**真的** CLI，
# 在 MON_DB_ENABLED=1 的機器上跑會真的寫進正式 `tech_users` 表——正式 DB 裡那筆
# `pkh_mapped@photons.com.tw / Mapped Guy / id4` 就是這樣來的（2026-09-16 清除）。
# 現在連寫入都由 stub 承接，測試不可能碰到任何真實資料。
#
# 分工：--set/--unset 的語意（NOOP／CONFLICT／ERR_NO_EMAIL）屬 registry CLI，
# 由 telegram-dispatcher/lib/registry/tech-users-sync.test.ts 覆蓋；本檔只驗
# 本腳本自己的責任——正確委派、原樣轉印 CLI 輸出、SET_OK 才重啟、CLI 失敗
# fail-loud，以及 --list 的比對邏輯。
set -uo pipefail

HERE="$(cd "$(dirname "$0")" && pwd)"
SCRIPT="$HERE/tg-map-chatids.sh"

PASS=0; FAIL=0
ok(){ PASS=$((PASS+1)); echo "ok   - $1"; }
no(){ FAIL=$((FAIL+1)); echo "FAIL - $1"; [ -n "${2:-}" ] && echo "        $2"; }
assert_eq(){ if [ "$2" = "$3" ]; then ok "$1"; else no "$1" "expected[$3] got[$2]"; fi; }
assert_has(){ if printf '%s' "$2" | grep -q "$3"; then ok "$1"; else no "$1" "[$2] missing [$3]"; fi; }
assert_no(){ if printf '%s' "$2" | grep -q "$3"; then no "$1" "[$2] should NOT contain [$3]"; else ok "$1"; fi; }

TMP="$(mktemp -d)"
trap 'rm -rf "$TMP"' EXIT

# ───────────────────────── 假 registry CLI ─────────────────────────
# 名冊固定四人；「已連接」的只有 777（給 --list / --check-connected 用）。
# --set/--unset 依 email 回固定的結果字串，模擬真 CLI 的輸出契約。
FAKE_CLI="$TMP/fake-registry.sh"
cat > "$FAKE_CLI" <<'EOF'
#!/usr/bin/env bash
case "${1:-}" in
  --list-roster)
    printf '%s\n' \
      'notion_user_name,notion_user_id,email,pushed_repos' \
      '洋蔥,id1,pkh_farus@photons.com.tw,abu' \
      'Dup One,id2,pkh_dup@photons.com.tw,abu' \
      'Dup Two,id3,pkh_dup2@photons.com.tw,abu' \
      'Mapped Guy,id4,pkh_mapped@photons.com.tw,abu'
    ;;
  --check-connected)
    shift
    for cid in "$@"; do
      if [ "$cid" = "777" ]; then echo CONNECTED; else echo NOT_CONNECTED; fi
    done
    ;;
  --set)
    case "${2:-}" in
      pkh_farus@photons.com.tw)  echo "SET_OK: ${2}" ;;
      pkh_mapped@photons.com.tw) if [ "${4:-}" = "--force" ]; then echo "SET_OK: ${2}"; else echo "SET_CONFLICT: ${2} 已有不同的 chat_id；需 --force"; fi ;;
      *)                          echo "SET_ERR_NO_EMAIL: ${2:-}" ;;
    esac
    ;;
  --unset)
    case "${2:-}" in
      pkh_mapped@photons.com.tw) echo "UNSET_OK: ${2}" ;;
      pkh_farus@photons.com.tw)  echo "UNSET_NOOP: ${2}（已經沒有 chat_id）" ;;
      *)                          echo "UNSET_ERR_NO_EMAIL: ${2:-}" ;;
    esac
    ;;
  *) echo "fake-registry: unsupported ${1:-}" >&2; exit 2 ;;
esac
EOF
chmod +x "$FAKE_CLI"
export TG_REGISTRY_CLI="bash $FAKE_CLI"

# ───────────────────────── --set ─────────────────────────
echo "## --set"

# TG_RESTART_CMD 一律 stub 掉——測試絕不能真的打 launchctl 去重啟正式服務。
# 1) CLI 回 SET_OK → 原樣轉印，且嘗試重啟（stub 成功 → RESTART_OK）
out="$(TG_RESTART_CMD="true" bash "$SCRIPT" --set pkh_farus@photons.com.tw 111)"
assert_has "set ok prints SET_OK" "$out" "SET_OK"
assert_has "set ok triggers RESTART_OK (stub)" "$out" "RESTART_OK"

# 1b) 重啟 stub 失敗 → SET_OK 仍算成功，但印 RESTART_WARN 不印 RESTART_OK
out="$(TG_RESTART_CMD="false" bash "$SCRIPT" --set pkh_farus@photons.com.tw 111)"
assert_has "set with failing restart still prints SET_OK" "$out" "SET_OK"
assert_has "set with failing restart prints RESTART_WARN" "$out" "RESTART_WARN"
assert_no  "set with failing restart does not print RESTART_OK" "$out" "RESTART_OK"

# 2) CLI 回 SET_CONFLICT → 不觸發重啟
out="$(TG_RESTART_CMD="true" bash "$SCRIPT" --set pkh_mapped@photons.com.tw 888)"
assert_has "set conflict prints SET_CONFLICT" "$out" "SET_CONFLICT"
assert_no  "set conflict does not restart" "$out" "RESTART"

# 3) --force 原樣傳給 CLI → SET_OK，且觸發重啟
out="$(TG_RESTART_CMD="true" bash "$SCRIPT" --set pkh_mapped@photons.com.tw 888 --force)"
assert_has "set force prints SET_OK"  "$out" "SET_OK"
assert_has "set force triggers RESTART_OK (stub)" "$out" "RESTART_OK"

# 4) CLI 回 SET_ERR_NO_EMAIL → 不觸發重啟
out="$(TG_RESTART_CMD="true" bash "$SCRIPT" --set nobody@photons.com.tw 999)"
assert_has "set unknown email prints SET_ERR_NO_EMAIL" "$out" "SET_ERR_NO_EMAIL"
assert_no  "set unknown email does not restart" "$out" "RESTART"

# 5) 參數不足 → 本腳本自己擋下，不呼叫 CLI
out="$(TG_RESTART_CMD="true" bash "$SCRIPT" --set pkh_farus@photons.com.tw)"
assert_has "set missing chat_id prints SET_ERR_ARGS" "$out" "SET_ERR_ARGS"

# ───────────────────────── --unset ─────────────────────────
echo "## --unset"

out="$(bash "$SCRIPT" --unset pkh_mapped@photons.com.tw)"
assert_has "unset existing prints UNSET_OK" "$out" "UNSET_OK"

out="$(bash "$SCRIPT" --unset pkh_farus@photons.com.tw)"
assert_has "unset empty prints UNSET_NOOP" "$out" "UNSET_NOOP"

out="$(bash "$SCRIPT" --unset nobody@photons.com.tw)"
assert_has "unset unknown email prints UNSET_ERR_NO_EMAIL" "$out" "UNSET_ERR_NO_EMAIL"

# ───────────────────────── --list-roster / --list-connected 轉發 ─────────────────────────
echo "## --list-roster"

out="$(bash "$SCRIPT" --list-roster)"
assert_has "list-roster 轉印 header" "$out" "notion_user_name,notion_user_id,email,pushed_repos"
assert_has "list-roster 轉印資料列"   "$out" "pkh_farus@photons.com.tw"

# ───────────────────────── registry CLI fail-loud（2026-09-02 委派後新增）─────────────────────────
echo "## registry CLI fail-loud"

# CLI 呼叫本身失敗（bun 不在／腳本炸掉）→ exit 1、印 *_ERR_REGISTRY、不重啟、不回退舊 awk 路徑
out="$(TG_RESTART_CMD="true" TG_REGISTRY_CLI="/usr/bin/false" bash "$SCRIPT" --set pkh_farus@photons.com.tw 111 2>&1)"; rc=$?
assert_eq  "set with broken CLI exits 1" "$rc" "1"
assert_has "set with broken CLI prints SET_ERR_REGISTRY" "$out" "SET_ERR_REGISTRY"
assert_no  "set with broken CLI does not restart" "$out" "RESTART"

out="$(TG_REGISTRY_CLI="/usr/bin/false" bash "$SCRIPT" --unset pkh_mapped@photons.com.tw 2>&1)"; rc=$?
assert_eq  "unset with broken CLI exits 1" "$rc" "1"
assert_has "unset with broken CLI prints UNSET_ERR_REGISTRY" "$out" "UNSET_ERR_REGISTRY"

out="$(TG_REGISTRY_CLI="/usr/bin/false" bash "$SCRIPT" --list-roster 2>&1)"; rc=$?
assert_eq  "list-roster with broken CLI exits 1" "$rc" "1"
assert_has "list-roster with broken CLI prints LIST_ROSTER_ERR_REGISTRY" "$out" "LIST_ROSTER_ERR_REGISTRY"

# ───────────────────────── --list ─────────────────────────
echo "## --list"

# getUpdates stub：模擬 5 個私聊 DM（含一個已連接 777、一個非私聊應被忽略）
cat > "$TMP/upd.sh" <<'STUBEOF'
#!/usr/bin/env bash
cat <<'JSON'
{"ok":true,"result":[
 {"update_id":1,"message":{"chat":{"id":111,"type":"private","first_name":"洋蔥","username":"farus422"}}},
 {"update_id":2,"message":{"chat":{"id":222,"type":"private","first_name":"Dup","username":"dupuser"}}},
 {"update_id":3,"message":{"chat":{"id":333,"type":"private","first_name":"Zzz","username":"zzz999"}}},
 {"update_id":4,"message":{"chat":{"id":777,"type":"private","first_name":"Mapped","username":"mapped"}}},
 {"update_id":5,"message":{"chat":{"id":-1009,"type":"group","title":"some group"}}}
]}
JSON
STUBEOF
chmod +x "$TMP/upd.sh"

# TG_UNKNOWN_SENDERS_LOG 指到不存在的檔案：隔離測試，不誤讀真實 production log
OUT="$(TG_GETUPDATES_CMD="$TMP/upd.sh" TG_UNKNOWN_SENDERS_LOG="$TMP/no-such.jsonl" bash "$SCRIPT" --list)"

line_of(){ printf '%s\n' "$OUT" | awk -F'\t' -v id="$1" '$1==id'; }
field(){ printf '%s' "$1" | awk -F'\t' -v n="$2" '{print $n}'; }

# first_name 完全相同且唯一 → HIGH + 正確 candidate_email
L111="$(line_of 111)"
assert_eq "111 confidence HIGH"      "$(field "$L111" 5)" "HIGH"
assert_eq "111 candidate_email"      "$(field "$L111" 6)" "pkh_farus@photons.com.tw"

# 對到兩列 → ASK
L222="$(line_of 222)"
assert_eq "222 confidence ASK"       "$(field "$L222" 5)" "ASK"
# 來源一律標記 getUpdates
assert_eq "222 source getUpdates"    "$(field "$L222" 2)" "getUpdates"

# 對不到 → ASK
L333="$(line_of 333)"
assert_eq "333 confidence ASK"       "$(field "$L333" 5)" "ASK"

# 已連接 chat_id（777，由 --check-connected 判定）→ 不出現在輸出（被跳過）
assert_no "connected 777 skipped"    "$OUT" "777"

# 非私聊（group -1009）→ 被忽略，不出現在輸出
assert_no "group chat ignored"       "$OUT" "1009"

# ── webhook-log 來源（telegram-dispatcher 的未知 sender log）──
echo "## --list（webhook-log 來源）"

# getUpdates 409 stub：模擬 webhook 掛著時的正常故障（不應中止，只警告）
cat > "$TMP/upd409.sh" <<'STUBEOF'
#!/usr/bin/env bash
echo "HTTP Error 409: Conflict" >&2
exit 1
STUBEOF
chmod +x "$TMP/upd409.sh"

cat > "$TMP/unknown-senders.jsonl" <<'JSONL'
{"ts":"2026-08-21T01:00:00.000Z","chat_id":"444","first_name":"洋蔥","last_name":"","username":"farus422"}
{"ts":"2026-08-21T01:05:00.000Z","chat_id":"555","first_name":"Zzz","last_name":"","username":"zzz999"}
JSONL

OUT2="$(TG_GETUPDATES_CMD="$TMP/upd409.sh" TG_UNKNOWN_SENDERS_LOG="$TMP/unknown-senders.jsonl" bash "$SCRIPT" --list)"
line2_of(){ printf '%s\n' "$OUT2" | awk -F'\t' -v id="$1" '$1==id'; }

# getUpdates 409（webhook 掛著時的正常情況）：不中止，仍能從 webhook-log 拿到結果
L444="$(line2_of 444)"
assert_has "getUpdates 409 不中止，仍有 webhook-log 結果" "$OUT2" "444"
assert_eq  "444 source webhook-log"  "$(field "$L444" 2)" "webhook-log"
assert_eq  "444 confidence HIGH"     "$(field "$L444" 5)" "HIGH"
assert_eq  "444 candidate_email"     "$(field "$L444" 6)" "pkh_farus@photons.com.tw"

L555="$(line2_of 555)"
assert_eq  "555 confidence ASK"      "$(field "$L555" 5)" "ASK"

# ── 名冊取不到 → --list 中止（不把「名冊空的」誤報成「所有人都待處理」）──
echo "## --list（名冊取不到）"
out="$(TG_REGISTRY_CLI="/usr/bin/false" TG_GETUPDATES_CMD="$TMP/upd.sh" TG_UNKNOWN_SENDERS_LOG="$TMP/no-such.jsonl" bash "$SCRIPT" --list 2>&1)"; rc=$?
assert_eq  "list with broken CLI exits non-zero" "$([ "$rc" -ne 0 ] && echo yes || echo no)" "yes"
assert_has "list with broken CLI prints LIST_ERR" "$out" "LIST_ERR"

# ── 分隔符正規化迴歸測試（2026-09-08：ting xuan / 天狼星 leon_chennnn 配對不上）──
echo "## --list（分隔符正規化迴歸測試）"

FAKE_CLI3="$TMP/fake-registry3.sh"
cat > "$FAKE_CLI3" <<'EOF'
#!/usr/bin/env bash
case "${1:-}" in
  --list-roster)
    printf '%s\n' \
      'notion_user_name,notion_user_id,email,pushed_repos' \
      'Ting-xuan TPE,id5,ptp_ting-xuan@photons.com.tw,abu' \
      'leon chen,id6,pkh_leon.chen@photons.com.tw,agrabah'
    ;;
  --check-connected)
    shift; for _ in "$@"; do echo NOT_CONNECTED; done ;;
  *) echo "fake-registry3: unsupported ${1:-}" >&2; exit 2 ;;
esac
EOF
chmod +x "$FAKE_CLI3"

cat > "$TMP/upd3.sh" <<'STUBEOF'
#!/usr/bin/env bash
cat <<'JSON'
{"ok":true,"result":[
 {"update_id":1,"message":{"chat":{"id":666,"type":"private","first_name":"Ting Xuan","username":""}}},
 {"update_id":2,"message":{"chat":{"id":888,"type":"private","first_name":"天狼星","username":"leon_chennnn"}}}
]}
JSON
STUBEOF
chmod +x "$TMP/upd3.sh"

OUT3="$(TG_REGISTRY_CLI="bash $FAKE_CLI3" TG_GETUPDATES_CMD="$TMP/upd3.sh" TG_UNKNOWN_SENDERS_LOG="$TMP/no-such2.jsonl" bash "$SCRIPT" --list)"
line3_of(){ printf '%s\n' "$OUT3" | awk -F'\t' -v id="$1" '$1==id'; }

# TG 顯示名「Ting Xuan」（空白分隔）比對名冊「Ting-xuan TPE」（連字號分隔）→ 應為 HIGH
L666="$(line3_of 666)"
assert_eq "666（Ting Xuan vs Ting-xuan）confidence HIGH" "$(field "$L666" 5)" "HIGH"
assert_eq "666 candidate_email"                          "$(field "$L666" 6)" "ptp_ting-xuan@photons.com.tw"

# TG username「leon_chennnn」（底線+多餘字尾）比對 email「leon.chen」（句點分隔）→ 應為 HIGH
L888="$(line3_of 888)"
assert_eq "888（leon_chennnn vs leon.chen）confidence HIGH" "$(field "$L888" 5)" "HIGH"
assert_eq "888 candidate_email"                             "$(field "$L888" 6)" "pkh_leon.chen@photons.com.tw"

# ───────────────────────── summary ─────────────────────────
echo "-----"
echo "PASS=$PASS FAIL=$FAIL"
[ "$FAIL" -eq 0 ]
