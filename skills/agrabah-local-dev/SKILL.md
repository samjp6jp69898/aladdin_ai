...
本機 DB 已有帳號資料（不需要、也不能用 `create-default-admin-user`，那個工具只在 `admin.users` 全空時能用），密碼是雜湊過的查不到明碼。**帳密需要向使用者確認**，不要自己猜或重設密碼（重設密碼是會動到既有帳號狀態的操作，需先問過）。
...
帳密（USER/PASS）需要向使用者索取，不要寫死進任何會留存的檔案。
