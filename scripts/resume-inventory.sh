...
#
# 用法：bash scripts/resume-inventory.sh <ticket_id>
#
...
#   3. 各主 repo `mr/{ticket}` 分支是否存在、是否有領先 origin/main 的 commit
...
#   REVIEW_B: ...                       REVIEW_C: ...
#   BRANCH_COMMITS: <repo>=<n> ...|none （只列分支存在的 repo；n = origin/main..mr/{ticket} commit 數）
...
for repo in agrabah abu lago rajah; do
  ...
  n=$(git -C "$ROOT/$repo" rev-list --count "origin/main..mr/$TICKET" 2>/dev/null || echo 0)
