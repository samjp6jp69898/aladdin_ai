---
name: tg-chatid-sync
description: Use when 需要把 DM 過 Telegram bot 的 chat_id 對映回填 tech_users 名冊的 tg_chat_id 欄、有人 DM 過 bot 但名冊還沒連接、pipeline 通知對某技術發不出去（TG_SKIP_NO_CHATID）、或新技術剛配對完要連結 Telegram 通知。
---

# tg-chatid-sync — Telegram chat_id 回填 tech_users 名冊
...
兩者皆**不依賴 telegram channel 的 `access.json`**（本流程與 channel bot 完全無關）。這些名稱**常常**能對到名冊但不保證，故核心原則是：**高信心自動寫入、不確定問使用者**。發現與寫入由 script 分離，流程與人工確認由本 skill 編排。

> **時效提醒：** webhook-log 來源沒有時間窗限制（log 持續累積、連接後 `--list` 自動跳過已連接者）。getUpdates 來源仍受 Telegram 近 **~24h** 未確認更新緩衝限制，但這只在 webhook 未掛著時才有意義。連接一旦寫入 `tech_users` 即永久保存（**DB 才是持久層**；2026-09-16 起 `tech-users.csv` 已刪檔退役）。

## 何時使用

- 「把有 DM 過 bot 的人對映到 tech_users 名冊」「回填 tg_chat_id」
- tg-notify.sh 對某人印 `TG_SKIP_NO_CHATID` → 該技術在名冊裡還沒連接 chat_id
...
| 指令 | 作用 |
|------|------|
| `bash scripts/tg-map-chatids.sh --list` | **唯讀**：合併 telegram-dispatcher 未知 sender log + bot 的 getUpdates，取得 DM 過的私聊 chat_id + 名稱，比對名冊（`--list-roster`）與連接狀態（`--check-connected`），輸出 TSV，不寫任何東西 |
...
1. 跑 `--list`，逐行處理：
   - **`confidence == HIGH`**（整份名冊恰好一個候選列）→ 直接 `--set <candidate_email> <chat_id>`，記入「自動對映」清單（仍會在報告列出供事後核對）。
...
對 getUpdates 取得的 `{first_name, last_name, username}`，逐名冊列判斷是否為「候選」（正規化＝去空白、轉小寫）：
...
測試訊息名稱預設用 getUpdates `first_name`（`Landon 連結成功` / `洋蔥 連結成功`，較口語）；若想改用名冊的 `notion_user_name` 可在 step 2 自行替換。

## 測試

`bash scripts/tg-map-chatids.test.sh`（全離線，stub getUpdates + fixture unknown-senders.jsonl + stub registry CLI，涵蓋 `--set` 各種結果的轉印與重啟行為、`--list` 的 HIGH / ASK / 已連接跳過 / 非私聊忽略，以及 getUpdates 409（webhook 掛著時的正常情況）不中止、仍從 webhook-log 拿到結果）。
