...
#
# 用法：
#   bash scripts/backtest-lock.sh claim <單號>    # 認領任務（原子操作）
#   bash scripts/backtest-lock.sh release <單號>  # 釋放鎖
#   bash scripts/backtest-lock.sh status <單號>   # 查詢鎖狀態
#   bash scripts/backtest-lock.sh cleanup         # 清理所有鎖
#   bash scripts/backtest-lock.sh list            # 列出所有鎖
#
# 退出碼：
#   0 = 成功
#   1 = 失敗（已被認領 / 參數錯誤）

LOCK_DIR="/tmp/backtest-locks"
...
case "$ACTION" in
    claim)
        if [ -z "$TICKET" ]; then
            echo "ERROR: 請提供單號，例如：bash scripts/backtest-lock.sh claim FAQ-1841"
            exit 1
        fi
    ...
    list)
        ...
        ;;
    *)
        echo "用法：bash scripts/backtest-lock.sh {claim|release|status|cleanup|list} [單號]"
        exit 1
        ;;
esac
