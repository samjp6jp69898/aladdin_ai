#!/usr/bin/env bun
/**
 * mcp-rajah-tasks-build-index.ts — 掃 obsidian/mcps/rajah-inventory/*.json（不含自己），
 ...
 *
 * 用法：bun obsidian/scripts/mcp-rajah-tasks-build-index.ts
 */

import { readFileSync, readdirSync, writeFileSync } from 'fs';

const DIR = '/Users/user/aladdin/obsidian/mcps/rajah-inventory';
...
function main() {
    ...
    const entries = files.map((f) => {
        ...
        return {
            ...
            json_path: `obsidian/mcps/rajah-inventory/${ f }`,
    ...
    writeFileSync(INDEX_FILE, JSON.stringify({
        ...
        schema_version: 1,
        note: '每個 files[] 項目對應一個 rajah/services/*.rajah 檔案的完整任務清單（obsidian/mcps/rajah-inventory/<stem>.json）。要重跑：先跑 mcp-rajah-tasks-gen.ts（可用 --files= 限定範圍），再跑本腳本重建這份 index。',
