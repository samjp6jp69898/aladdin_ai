...
async function main() {
    ...
    const lines: string[] = [
        ...
        `Total cross-server RPC edges (within Milestone 1 scope): **${edges.length}**`,
        '',
        '> 列出所有跨服務 RPC 呼叫邊。Milestone 1 範疇只涵蓋 4 個 server（wallet/app/core/payment）+ 2 個 manager；呼叫到範疇外的 server 會顯示為斷裂連結（可透過 `broken-links-report.md` 查閱）。',
        '',
        '## 統計：按 Caller Server',
        '',
    ];
