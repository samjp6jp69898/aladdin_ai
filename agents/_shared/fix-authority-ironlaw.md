...
5. **升格為根因定位通則 + 但書**：本 §C「源頭優先、禁 min-hop」原則經 bug-tracer Step 3「根因選擇通則 R1」升格 —— 不再只作用於「多顆同 ticket-id commit 仲裁」，而是所有根因定位（含同一 angle 內選哪個函式、跨 angle 選哪一側）的通則：主因恆歸「壞值 / 壞狀態首次被產生之處」。**但書（防矯枉過正）**：源頭側必須是「非預期 / 錯誤」，而非「設計上正確的預設值 / 既有契約」；若上游是合規設計（刻意設定的 DB 預設值、契約本身正確），主因仍在下游消費端。此但書同樣適用於本節 commit 仲裁，避免把「修了設計上正確的上游」的 commit 誤判為主因 commit。
